...
class MinimalPublisher2(Node):
    ...
    def test2(self):
        try:
            try:
                ...
                self.tf_tag25_to_robot = tf_tag_25_to_base_footprint #สำรองข้อมูล TF ไว้เมื่อเจอ tag
            ...
            if len(self.apriltag_msg.detections) == 3 and ((abs(self.tf_tag25_to_robot.transform.translation.x) >= self.min_dist_to_deatect_all_tags) or (self.first_point == True)):
                ...
                try: # use try to prevent loss data on current time
                    ...
                    if self.first_point == True :
                        ...
                        scale = (self.palabola_beta*(self.tf_tag25_to_robot.transform.translation.x + self.palabola_dist)**2) + self.palabola_alpha
                        # scale = -0.2
                ...
                if self.first_point == False:
                    try:
                        if abs(self.tf_tag25_to_robot.transform.translation.x) < self.min_dist_to_deatect_all_tags: #robot close to station < 0.4, 0.5 / 0.45 meters // maximum distance to detect 3 tags
                            ...
                            q = quaternion_from_euler(0.0, 0.0, 3.141592) #rpy = 0.0, 0.0, pi
    ...
    def test3(self):
        ...
        try:
            ...
            if abs(self.tf_tag25_to_robot.transform.translation.x) > self.docking_distance + self.distance_goal_tolerance_last_predocking :
                ...
                elif (math.fabs(yaw_goal_error) > self.yaw_goal_tolerance) and (abs(self.tf_tag25_to_robot.transform.translation.x) > self.yaw_adjust_disable_by_distance_x): #0.45 # ถ้าระยะมากกว่า x จะปรับ heading ตอนไปถึง goal  
    ...
    def timer_callback(self):
        ...
        if self.docking_state == 'init':
            ...
            self.test2()
            if self.time_ >= self.delay_before_start: # waitting
