...
class DemoNode():
    def __init__(self):
        ...
        # Declare angle metrics in radians
        self.heading_tolerance = self.heading_tolerance_
        self.yaw_goal_tolerance = self.yaw_goal_tolerance_
        ...
        try:
            ...
            if len(self.apriltag_msg.detections) == 3:
                ...
                try: # use try to prevent loss data on current time
                    ...
                    tf_robot_to_tag_left = self.tfBuffer.lookup_transform('starting_point', 'tag_24', rospy.Time()) #left
                    tf_robot_to_tag_middle = self.tfBuffer.lookup_transform('starting_point', 'tag_25', rospy.Time()) #middle
                    tf_robot_to_tag_right = self.tfBuffer.lookup_transform('starting_point', 'tag_26', rospy.Time()) #right
                    ...
                    try:
                        tf_tag_25_to_base_footprint = self.tfBuffer.lookup_transform('tag_25', self.base_footprint, rospy.Time()) #middle
                    except:
                        pass
                    else:
                ...
                if self.first_point == False:
                    try:
                        if abs(self.tf_tag25_to_robot.transform.translation.z) < 0.5:
    ...
    def demo_callback(self, timer):
        ...
        if self.docking_state == 'init':
            ...
            self.time_ = self.time_ + self.duration_time
            self.test2()
            if self.time_ >= 0.5: #prevent missing tf
            ...
            try:
                ...
                if self.first_point == True:
                    ...
                    elif abs(self.tf_tag25_to_robot.transform.translation.x) >= self.distance_goal_tolerance_first_predocking_ :
                        self.distance_goal_tolerance = self.distance_goal_tolerance_first_predocking_ 
                else:
                ...
                if (math.fabs(distance_to_goal) > self.distance_goal_tolerance and self.reached_distance_goal == False ):
                    ...
                    if (math.fabs(heading_error) > 0.2): #and abs(self.tf_tag25_to_robot.transform.translation.z) > 0.6: # > 0.5, 0.6 ถ้าระยะน้อยกว่า x จะไม่ปรับ major heading  
                        ...
                        # option 1
                        if (abs(self.desired_y - self.odom_current.pose.position.y) < 0.01) and abs(self.tf_tag25_to_robot.transform.translation.z) < 0.45: # < 0.45, 0.55 # ถ้าระยะน้อยกว่า x จะไม่ปรับ heading
                ...
                elif (math.fabs(yaw_goal_error) > self.yaw_goal_tolerance) and (abs(self.tf_tag25_to_robot.transform.translation.z) > self.docking_distance + self.distance_goal_tolerance_next_predocking_) : # ถ้าระยะมากกว่า x จะปรับ heading ตอนไปถึง goal  
                ...
                # Goal achieved, go to the next goal  
                    ...
                    if abs(self.tf_tag25_to_robot.transform.translation.z) <= self.docking_distance + self.distance_goal_tolerance : # 0.X + tolerance
                        try:
