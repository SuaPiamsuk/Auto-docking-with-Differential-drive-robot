        ...
        try:
            if len(self.apriltag_msg.detections) == 3:
                ...
                try: # use try to prevent loss data on current time
                    ...
                    if self.first_point == True and (self.tf_robot_to_tag25.transform.translation.x < -1.0 or self.tf_robot_to_tag25.transform.translation.x > -0.9 or abs(self.tf_tag25_to_robot.transform.translation.x)>0.05 ):
    ...
    def demo_callback(self, timer):
        ...
        if self.docking_state == 'init':
            ...
            try:
                ...
                #     # if abs(robot2predocking_2.transform.translation.y) > 0.01:
                #     #     self.direction = 1.0
                ...
                # if self.first_turnaround == True: #fixed direction
                #     if robot2predocking_2.transform.translation.x >= 0.0 :
                #         self.direction = -1.0
                #     elif robot2predocking_2.transform.translation.x < 0.0 :
                #         self.direction = 1.0

                #     self.first_turnaround = False
                ...
                #     robot2predocking_2 = self.tfBuffer.lookup_transform('pre_docking', 'starting_point', rospy.Time())
                #     _, _, yaw_robot_predocking = euler_from_quaternion(robot2predocking_2.transform.rotation.x , robot2predocking_2.transform.rotation.y, robot2predocking_2.transform.rotation.z, robot2predocking_2.transform.rotation.w)
                #     # robot2predocking_2.transform.translation.x = round(robot2predocking_2.transform.translation.x,1)
                #     # self.direction = (math.copysign(1, robot2predocking_2.transform.translation.x))
                #     if robot2predocking_2.transform.translation.x > 0:
                #         self.direction = 1.0
                #     elif robot2predocking_2.transform.translation.x <0:
                #         self.direction = -1.0
                #     else:
                #         self.direction = -1.0
                ...
                # Goal achieved, go to the next goal  
                    ...
                    # 'Arrived at perpendicular line. Going straight to ARTag...'
                    self.reached_distance_goal = False   
